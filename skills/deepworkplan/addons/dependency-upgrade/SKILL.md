...
description: Optional DeepWorkPlan addon that safely upgrades a repo's dependencies — reasoning about the repo's ACTUAL package manager (npm/pnpm/yarn + ncu, pip/poetry/uv, cargo, go mod, bundler, composer, and more) rather than assuming npm — with a batched, validated, revertible workflow: detect manager and manifests/lockfiles, classify upgrades (patch/minor/major), upgrade in safe batches, run the repo's real validation gate after each batch, revert a failing batch, and summarize. Opt-in, never required, reconciles with the repo's existing tooling. Use when the developer wants to bring dependencies up to date without breaking the build.
version: "2.0.2"
